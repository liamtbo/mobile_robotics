...
class Mapping(Node):
    ...
    def _odom_callback(self, msg):
        ...
        self.get_logger().info(f'odom: ({msg.pose.pose.position.x}, {msg.pose.pose.position.y})')
    ...
    def _scan_callback(self, scan):
        ...
        nan_idxs = np.isnan(scan_ranges)
        scan_ranges[nan_idxs] = np.median(scan_ranges[~nan_idxs])
        scan_ranges[scan_ranges > 3] = 3
        ...
        self.occ_grid_rviz.header.stamp = self.get_clock().now().to_msg()
    ...
    def update_probs(self, scan):
        ...
        # i = len(scan.ranges) // 2
            ...
            # self.get_logger().info(f'occ_laser_cells_x: {np.max(occ_laser_cells_x)}')
            occ_laser_cells_y = occ_laser_cells[:,1]
            # self.get_logger().info(f'occ_laser_cells_y: {np.max(occ_laser_cells_y)}')
